...
import { headerVariants, socialLinksVariants } from './Header.variants';
...
export const SocialLinks: React.FC<{
  ...
  const styles = socialLinksVariants({ variant });

  return (
    <div className={styles.container()}>
      <ul className={styles.list()}>
        {links.map((social) => (
          <li key={social.platform} className={styles.item()}>
            <a
              href={social.href}
              target="_blank"
...
export const Header: React.FC<HeaderProps> = ({
  logoSrc = '/images/common/site-logo.svg',
  logoAlt = 'Ride Like a Pro logo',
  logoHref = '/',
  navItems = [],
  socialLinks = [],
  variant = 'default',
  ...
  const styles = headerVariants({ variant, isMenuOpen });
      ...
      <header className={styles.header()}>
        <div className={styles.headerMain()}>
          ...
          <div className={styles.logoWrapper()}>
            <Link href={logoHref} className={styles.logoLink()} title={logoAlt}>
              <Image
                src={logoSrc}
                alt={logoAlt}
                width={180}
          ...
          <div className={styles.headerContainer()}>
            ...
            {/* Desktop Right Section: Social + Nav stacked */}
            <div className={styles.headerRightSection()}>
              {/* Social Links - Desktop Only */}
              {socialLinks.length > 0 && (
                <div className={styles.socialWrapper()}>
                  <SocialLinks links={socialLinks} variant={socialVariant} />
                </div>
              )}
